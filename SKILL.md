...
- Naming conventions:
  ...
  - Never say "directory" or "dir". Say "folder" instead. A path value declares its own kind via its type. Trailing / means folder, no trailing / means file. A path value always starts with ./. A leading . right after that (dotfile) is fine.
...
- enum_ref is a value-source modifier, not a type and not part of any type's contract below. It restricts the arg's value or values to one Shared Enum's members instead of inlining them, optional on any type.
### Path
- Two distinct types. Folder: type: path-folder, trailing /. File: type: path-file, no trailing /.
- Always starts with ./. A leading . right after that (dotfile) is fine.
- A bare ./, a bare ., and a bare / are all invalid.
- Never contains .., a repeated /, or a redundant ./ segment.
- Always resolves to one exact folder or file, never an abstract stand-in for "current" or "any" location. Glob and regex are the only types allowed to express that kind of abstract or pattern-based location.
- No literal value expresses the project root. An optional arg of this type defines omission itself as meaning root.
- Declaring type: path-file or type: path-folder is the enforcement. The runner rejects a value that doesn't start with ./, a bare ./, a bare ., a bare /, a value containing .., a repeated /, a redundant ./ segment, or a wildcard character, and confirms the trailing-/ shape matches the declared type, as a run prerequisite before the script's command ever executes. A command must never re-check or restate this itself.
...
### Glob
- Matched against a basename only, never the full path. Not restricted to files, a folder's basename matches the same way.
### Bool
