#!/usr/bin/env bash
# Atomic step: bump the canonical Formula/bumfuzzle.rb (kept in the sibling
# arc-com/homebrew-tools repo, not this one) to VERSION, then commit and push.
# Safe to run standalone to retry just this step after a partial release.
#
# Does not touch the Homebrew tap - that's release-homebrew-tap.sh, run
# separately (by release.sh, right after this) so a tap-push failure can be
# retried on its own without re-bumping the formula.
set -euo pipefail
...
echo "==> $FORMULA bumped to v$VERSION and pushed"
